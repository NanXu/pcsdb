...
$(function() {
	...
	$("#userAddButton").click(function() {
		var url = "${ctx}/system/user/toAddUser";
	...
	$("#userRoleButton").click(function() {
		...
		if(msg==""){
			window.parent.topWin.Dialog.alert("请选择要分配角色的用户");
		} else {
			var array = msg.split(",");
			if(array.length == 2) {
				var url = "${ctx}/system/user/toUserRoleAssign/"+ array[1];
				window.parent.topWin.Dialog.open({URL:url,Title:"用户角色分配",Width:640,Height:400});
			} else {
				window.parent.topWin.Dialog.alert("只能选择一个用户进行角色分配");
			}
		}
	});
});
...
function operate(id, status) {
	...
	window.parent.topWin.Dialog.confirm(msg,function(){
		var url = "${ctx}/system/user/forbid/"+id+"/"+status;
...
<body>
	<form action="${ctx}/system/user/listUsers" id="searchForm" method="post">
	...
	<div class="box2" panelTitle="用户信息列表">
		<div style="height:35px;">
			<div class="float_left padding5">
				...
				<button type="button" id="userRoleButton"><span class="icon_user_group">用户分配角色</span></button>&nbsp;
			</div>
